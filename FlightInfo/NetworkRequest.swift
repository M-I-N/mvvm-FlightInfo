...
//    func fetch(with request: URLRequest, decode: (Decodable)-> Model?, completion: @escaping (Result<Model, APIError>)-> ())
}
    ...
    private func decodingTask(with request: URLRequest, completion: @escaping (Decodable?, APIError?)-> ())-> URLSessionDataTask {
        let task = session.dataTask(with: request) { (data, response, error) in
            ...
            if response.statusCode == 200 {
                if let data = data {
                    do {
                        let genericModel = try JSONDecoder().decode(Model.self, from: data)
